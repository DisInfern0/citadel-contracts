...
describe("citadel nft", function () {
  ...
  describe("minting", function () {
    ...
    it("update merkle root, mint citadel acct 4", async function () {
      ...
      var merkleRoot = await this.citadel.getMerkleRoot();
      expect(merkleRoot).to.equal(
        "0x3d3e1b17235644ae1fbabbdada2fa910e008474cb0d1e9ec310216cd41ed1c35"
      );
      ...
      merkleRoot = await this.citadel.getMerkleRoot();
      expect(merkleRoot).to.equal(
        "0xfc75de997a507fbe5e55a49259e30e7670c36f09677014b2f434bf6a5ccea7ba"
      );
  ...
  describe("administration", function () {
    it("pauses minting", async function () {
      ...
      await this.citadel.pause();
      await expectRevert(
        this.citadel.mintCitadel(this.hexProofAcct1),
        "ERC721Pausable: token transfer while paused"
      );
      await this.citadel.unpause();
    });

    it("pauses minting, unpauses minting, mints", async function () {
      [owner, addr2] = await ethers.getSigners();
      const tokenId = await this.citadel.totalSupply();
      await this.citadel.pause();
      await this.citadel.unpause();
    ...
    it("adds dev address", async function () {
      ...
      await this.citadel.addDevRole(addr2.address);

      await this.citadel.reserveCitadel(10);

      //uplevel citadel with new dev role
      await this.citadel.connect(addr2).changeLevel(1, 1);
    ...
    it("reverts add dev role from unauthed address", async function () {
      ...
      await expectRevert(
        this.citadel.connect(addr2).addDevRole(addr2.address),
  ...
  describe("citadel utility", function () {
    it("uplevels a citadel on-chain", async function () {
      ...
      await expectRevert(
        ...
        "must have dev role to uplevel citadel"
      );
    });
  });
});
